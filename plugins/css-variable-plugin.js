...
const cssVariablePlugin = plugin.withOptions(
  ...
  (options = {}) => {
    ...
    if (debug) {
      console.log('[CSS Variable Plugin] Configuration received:', JSON.stringify(variables, null, 2));
    }
    ...
    const scanCssVariables = (filePath) => {
      try {
        ...
        if (debug) {
          ...
          console.log('[CSS Variable Plugin] Sample variables:', Object.keys(foundVariables).slice(0, 10));
        }

        return foundVariables;
      } catch (error) {
        if (debug) {
          console.log(`[CSS Variable Plugin] Could not read CSS file ${filePath}:`, error.message);
        }
        return {};
      }
    };
    ...
    if (autoScan) {
      ...
      for (const filePath of possiblePaths) {
        ...
        if (Object.keys(scannedVariables).length > 0) {
          if (debug) {
            console.log(`[CSS Variable Plugin] Successfully read CSS file from: ${filePath}`);
          }
          break;
        }
      }
      ...
      if (Object.keys(scannedVariables).length === 0 && debug) {
        console.log('[CSS Variable Plugin] Could not find CSS file at any of these paths:', possiblePaths);
      }
    }
    ...
    const processPatterns = (scannedVars, patterns) => {
      ...
      Object.entries(patterns).forEach(([themeKey, pattern]) => {
        ...
        const matchedVars = Object.keys(scannedVars).filter((varName) => regex.test(varName));
        ...
        if (matchedVars.length > 0) {
          ...
          if (debug) {
            ...
            console.log(`[CSS Variable Plugin] Matched variables:`, matchedVars.slice(0, 5));
          }
        }
      });
    ...
    const autoGeneratedVariables = autoScan ? processPatterns(scannedVariables, patterns) : {};
    ...
    if (debug && autoScan) {
      ...
      console.log('[CSS Variable Plugin] Final merged variables:', JSON.stringify(allVariables, null, 2));
    }
    ...
    Object.entries(allVariables).forEach(([themeKey, variableList]) => {
      ...
      if (Array.isArray(variableList)) {
        if (debug) console.log(`[CSS Variable Plugin] Processing array format for ${themeKey}`);
        ...
        variableList.forEach((variable) => {
          ...
          if (debug) console.log(`[CSS Variable Plugin] Generated: ${key} => ${value}`);
        });
      } else if (typeof variableList === 'object') {
        if (debug) console.log(`[CSS Variable Plugin] Processing object format for ${themeKey}`);
    ...
    if (debug) {
      console.log('[CSS Variable Plugin] Final theme extensions:', JSON.stringify(themeExtensions, null, 2));
    }
